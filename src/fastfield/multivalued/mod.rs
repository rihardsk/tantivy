...
mod tests {
    ...
    fn test_multivalued_date() {
        ...
        // add another second
        let three_secs_ahead = first_time_stamp + Duration::seconds(3);
        index_writer.add_document(doc!(date_field=>two_secs_ahead, date_field=>three_secs_ahead,date_field=>two_secs_ahead, time_i=>4i64));
        assert!(index_writer.commit().is_ok());
